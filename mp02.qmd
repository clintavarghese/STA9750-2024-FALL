...
I believe everyone enjoys a good movie, though what defines a "good film" can vary from person to person—a debate in itself. In this project, I aim to conduct an exploratory data analysis using the [the IMDb
non-commercial release](https://datasets.imdbws.com/) to uncover insights into movie popularity, ratings, and trends in genre popularity across decades. Ultimately, my goal is to propose a movie remake based on these findings.
...
In simple terms, when I watch a movie, there are two outcomes: either I like it or I don’t. I add my vote as +1 and 2 to the denominator. This method uses a simplified Bayesian average to handle uncertainty in ratings with few votes. The formula is:
The formula used is:
...
bio_anime_movies <- bio_anime_movies |>filter(vote_quantile>3)|>select(-averageRating,-startYear)|>
  ...
  distinct()
sample_n(bio_anime_movies|>select(-X)|>arrange(desc(numVotes),desc(new_rating)),100)|>DT::datatable()
```
...
```{r}
bio_anime_movies|>filter(grepl("Lawrence of Arabia", primaryTitle))|>select(primaryTitle,numVotes,new_rating,decade)|>distinct()
```
...
![](docs/movie.png){fig-align="center"}
