...
The [MIT Election Data Science Lab](https://electionlab.mit.edu/) collects votes from all biennial congressional races in all 50 states.The [US President Election]() and [House Of Representative Elections]() Dataset are downloaded.  
...
#### 1.1976 to 2012 Congression Boundaries
...
##### ii.2014 to Present Boundary Files

```{r}
#|warning: false
#|message: false
#|results: "hide" 
# Check and load tigris package
if(!require('tigris'))install.packages('tigris')
library(tigris)
library(leaflet)
cd_list <- list()
cd_files <- sprintf("cd%03d", 113:124)
year <- 2013

for (cd_file_name in cd_files) {
  
  if (year<2024)
    {
    
  # Try to fetch congressional district data
  cd <- congressional_districts(cb = TRUE, resolution = '20m', year = year)
  
  # Store data in the list
  cd_list[[cd_file_name]] <- cd
  }
  # Increment year for the next iteration
  year <- year + 1
  
}

US_CONGRESSIONAL_BOUNDARIES2<-cd_list
...
seat_changes|>select(state,seat_count_1976,seat_count_2022,change)|>
  arrange(change)|>slice(1:5)
```

```{r}
seat_changes|>select(state,seat_count_1976,seat_count_2022,change)|>
  arrange(desc(change))|>slice(1:5)
```
...
```
The results would have been different if it was not introduced.
...
```
using `con_boundaries FUNCTION` function, I read the .shp files from the zip content and are stores as a list.
...
presidential_votes2000<-presidential_votes2000|>group_by(state,candidate)|>slice_max(candidatevotes)|>select(state,candidate,party_simplified,candidatevotes,totalvotes)|>
...
us_states_2000<-us_states_2000|>filter(!is.na(geometry), !is.na(candidate))|>left_join(total_seats,by=c("STATENAME"="state"))|>
  mutate(state_centroid = st_centroid(geometry)) |>
  mutate(
    centroid_x = st_coordinates(state_centroid)[, 1],  # Extract x-coordinates
    centroid_y = st_coordinates(state_centroid)[, 2]   # Extract y-coordinates
  )
...
```{r}
# Plot the data
ggplot(us_states_2000) +
  geom_sf(aes(fill = party_simplified), color = "white") +
  geom_text(aes(x = centroid_x, y = centroid_y, label = no_of_seats), size = 3, color = "black") +
  scale_fill_manual(values = c("DEMOCRAT" = "blue", "REPUBLICAN" = "red")) +
  coord_sf(xlim = c(-130, -60), ylim = c(24, 50))  +
  labs(fill = "Winning Party",
...
```{r}
#| label: fig-charts
#| fig-cap: "Election Results Over Time"
#| layout-ncol: 2
#| fig-width: 15
#| fig-height: 10
...
presidentials_winners_combined_sf<-presidential_winners_by_state|>select(year,state,party_simplified)|>left_join(us_states_sf|>select(STATENAME,geometry)|>mutate(STATENAME=toupper(STATENAME)),by=c("state"="STATENAME"))
...
facet_wrap(~year, ncol = 2, scales = "fixed")  # `ncol = 2` ensures two maps per row
